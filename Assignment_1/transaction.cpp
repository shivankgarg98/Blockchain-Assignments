...
bool txn_t::validate() 
{	
	uint8_t pk_hash[SHA256_DIGEST_LENGTH], t_hash[SHA256_DIGEST_LENGTH];

	SHA256_CTX sha256;
   	SHA256_Init(&sha256);
   	SHA256_Update(&sha256, public_key.data(), public_key.size());
    SHA256_Final(pk_hash,&sha256); //optimisation1-don't explicitly calculate hash of public_key
    SHA256_Update(&sha256, source_addr.data(), source_addr.size());
   	SHA256_Update(&sha256, dest_addr.data(), dest_addr.size());
   	SHA256_Update(&sha256, change_addr.data(), change_addr.size());
   	SHA256_Update(&sha256, &amount, sizeof(amount));
    ...
    // TODO: implement this. -- This is Homework Problem 1
	// std:: cout <<"------------------------------------------------------" <<  source_addr <<  h1 <<"----------------------------------" ;
	if(source_addr != pk_hash) return false;
	if(tx_hash != t_hash ) return false;
	rsa_public_key_t puk(public_key.data(), public_key.size());
	bool ver = puk.verify((const uint8_t*)tx_hash.data(), tx_hash.size(), tx_sign.data(), tx_sign.size());
...
{
    auto pos = balances.find(source_addr);
    assert (pos != balances.end()); // NDEBUG is defined
    assert (pos->second >= amount); // "   "   "    " 
    //TO_DO find source address in balance_map 
    //unordered_map documentation
      
    
    if(pos == balances.end() || pos->second < amount)
        return;
     //pos.second gives mapped value = amt( of Type T = uint64_t )
      
    // calculate change.
    uint64_t change = pos->second - amount; // if pos->second  < amount then return
  
    balances[source_addr] = 0; // source has no more balance.

    if(balances.find(change_addr) == balances.end()) // search for the address where change is to be sent
        balances[change_addr] = change;
    else 
        balances[dest_addr] += change;
    
    if(balances.find(dest_addr)==balances.end()) //search if the addr already exists
        balances[dest_addr] = amount;
    else
        balances[dest_addr] =  balances[dest_addr] + amount;
    ...
    if(balances.find(change_addr) == balances.end()) // search for the address where change is to be sent
        ...
        balances[dest_addr] += change; //
        //balances.clear(pos);
     balances.erase(pos); //erase the element - save memory
...
bool txn_t::read(std::istream& in)
{
    ...
    // Check for sizes of key and signature 
