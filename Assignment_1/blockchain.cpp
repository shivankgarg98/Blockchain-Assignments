...
bool myvalidate_block_helper(
        ...
        const uint64_t BLOCK_reward
   	    ...
   	    SHA256_CTX sha256_blk;
	    SHA256_Init(&sha256_blk);
		

		vector <shared_ptr <txn_t> > :: iterator ptr;
	    for(ptr = transactions.begin(); ptr < transactions.end(); ptr++){
            
            if((*ptr)->validate() == true) //if transaction is valid then proceed
			    	(*ptr)->update_balances(balances);
	
        //check the transaction hash of current iteration
        	SHA256_CTX sha256_trans;
		    SHA256_Init(&sha256_trans);
		    SHA256_Update(&sha256_trans, (*ptr)->public_key.data(), (*ptr)->public_key.size());
    		SHA256_Update(&sha256_trans, (*ptr)->source_addr.data(), (*ptr)->source_addr.size());
    		SHA256_Update(&sha256_trans, (*ptr)->dest_addr.data(), (*ptr)->dest_addr.size());
    		SHA256_Update(&sha256_trans, (*ptr)->change_addr.data(), (*ptr)->change_addr.size());
    		SHA256_Update(&sha256_trans, &((*ptr)->amount), sizeof((*ptr)->amount));
		    
            hash_result_t trans;
            SHA256_Final(const_cast<uint8_t*>(trans.data()), &sha256_trans);
			
        // update transacation_hash in block_hash Pointer
		    SHA256_Update(&sha256_blk, trans.data(), trans.size());
	    }

	    SHA256_Update(&sha256_blk, reward_addr.data(), reward_addr.size());
        SHA256_Update(&sha256_blk, prev_hash.data(), prev_hash.size());
	    
        hash_result_t block_hash_current;
        SHA256_Final(const_cast<uint8_t*>(block_hash_current.data()), &sha256_blk);
        
        bool isValid = 0; //assign initially as 0
        if(blk_hash == block_hash_current)  //if hashes are equal then it is VALID block
            isValid = 1;

        if(isValid == 1){
            if(balances.find(reward_addr)==balances.end()) //if reward_addr is not available in balances
                balances[reward_addr] = BLOCK_REWARD; //total balance is reward
            else
                balances[reward_addr] =  balances[reward_addr] + BLOCK_REWARD; //total balance is sum of existing and reward 
                }
...
bool block_t::validate()
{
    ...
    return myvalidate_block_helper(
...
bool block_t::add_transaction(std::shared_ptr<txn_t> tx, bool check_tx)
{
    ...
    if (check_tx) {
        // update the balances.
        tx->update_balances(balances); //NOTE -> THIS IS HOW THEY UPDATE BALANCES //
	
    }
    // retu`rn true if added.
    return true;
}
